...
使用mysql时，将多行数据转成一行数据，考虑使用group_concat函数，

**使用group_concat函数的注意事项：**

```
如果使用group_concat函数后，使用having 进行条件匹配的话，可能会出现顺序不一致问题，
也就是group_concat组装后的字符顺序和你having中的字符顺序不一致.
```

将一行转换成多行数据，考虑使用find_in_set函数。

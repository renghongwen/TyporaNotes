...
```
redo日志和undo日志都是存储引擎层生成的日志，但是redo日志记录的是“物理级别”上的页修改操作，比如：页大小，页偏移量等参数的记录，而undo日志记录的是逻辑操作日志，比如对一行数据进行了insert语句，在undo日志中记录的是与之相反的delete操作，用于回滚操作和一致性非锁定读（mvcc:undo 日志回滚行记录到某种特定的版本）。
```
